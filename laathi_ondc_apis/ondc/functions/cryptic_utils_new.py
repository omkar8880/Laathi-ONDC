...
f = open(os.getenv("REQUEST_BODY_PATH", "request.txt"), "r")
...
def create_signing_string(digest_base64, created=None, expires=None):
    ...
    signing_string = f"""(created): {created}(expires): {expires}digest: BLAKE-512={digest_base64}"""
...
def create_authorisation_header(request_body=request_body_raw_text, created=None, expires=None):
    ...
    subscriber_id = os.getenv("SUBSCRIBER_ID", "laathi.com")
    unique_key_id = os.getenv("UNIQUE_KEY_ID", "731")
...
def encrypt(crypto_private_key, crypto_public_key, null):
  ...
  # request_body_raw_text = "Your string content"  # Replace with your actual string
    text_with_quotes = str(request_body_raw_text)
    # Create a byte string from the string
    mytext = text_with_quotes.encode('utf-8')
    mytextv2 = mytext.replace(b'\n', b'')
    # Place the byte string between single quotes
    
    logging.debug(f"Value of text_with_quotes: {text_with_quotes}")
    logging.debug(f"Value of mytextv2: {mytextv2}")
    #print(text_with_quotes)

    return base64.b64encode(cipher.encrypt(pad(mytextv2,AES.block_size))).decode('utf-8')
